...
lns1 = ax1.plot(tt,xx_1*1000,'b',label='unexcited, undamped')
lns2 = ax1.plot(tt,xx_2*1000,'b--',label='excited, undamped')
ax1.set_xlabel('time (s)')
# Make the y-axis label, ticks and tick labels match the line color.
ax1.set_ylabel('amplitude (mm)', color='b')
ax1.tick_params('y', colors='b')


ax2 = ax1.twinx()
s2 = np.sin(2 * np.pi * t)
lns3 = ax2.plot(tt,xx_forcing*1000,'r:',label='forcing function',zorder=0)
...
ax2.legend(lns, labs, loc=0, framealpha=1)

fig.tight_layout()
plt.show()

plt.savefig('figures/forced_unforced_plot.png',dpi=300)

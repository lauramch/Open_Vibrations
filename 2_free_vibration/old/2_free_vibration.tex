...
\begin{document}
	...
	\subsection{Mathematical Modeling of Free Vibration}
		...
		\subsubsection{Solve for the natural frequency ($\omega_n$) of the system }
			...
			Using these four constituent components obtained by observing simple harmonic motion, an equation can be proposed: 
			\begin{equation}
				{x}(t) = A\text{sin}(\omega_n t + \phi)
			\end{equation}
			Take the derivative with respect to time ($t$) to get velocity:
			\begin{equation}
			...
			\end{equation}
			Substituting $x$ and $\ddot{x}$ into the EOM for the considered 1-DOF system ($m\ddot{x} + kx = 0$) and dropping the ``($t$)'' for simplicity yields:
			\begin{equation}
			...
			\end{equation}
			This form is not an ODE so the equation is solvable and testable. This equation relates the frequency of a system to stiffness and mass. This equation leads to:
			\begin{equation}
				T= \frac{2 \pi}{\omega_n}
			\end{equation}
		...
		\subsubsection{Solve for initial phase ($\phi$) of the system }
			The EOM is a second-order ODE so we need two constants (initial conditions) to solve it. Here we will use the displacement ($x$) and velocity ($\dot{x}$ or $v$) at $t=0$. For simplicity, these are written as 
			\begin{equation}
				x(0) = x_0  
			\end{equation}			
			\begin{equation}
			...
			\end{equation}
			Now that we can derive mathematical meanings for $\phi$ and $A$. To do this, $\phi$  can be solved for by rearranging equations \ref{eq:x_0=Asintheta} and \ref{eq:v_0=Aomegacosphi} for $A$:
			\begin{equation}
		...
		\subsubsection{Solve for amplitude ($A$) of the system }
			...
			\end{equation}
			knowing that $\text{sin}(\phi)^2+\text{cos}(\phi)^2=1$ :
			\begin{equation}
				\bigg(\frac{x_0}{A}\bigg)^2  + \bigg(\frac{v_0}{\omega_nA}\bigg)^2 = 1
			\end{equation}
			and multiplying each side by 1 (also expressed as $\frac{\omega_n}{\omega_n}$), gives us the equation:
			\begin{equation}
				\bigg(\frac{\omega_n}{\omega_n}\bigg)^2\bigg(\frac{x_0}{A}\bigg)^2  + \bigg(\frac{v_0}{\omega_nA}\bigg)^2 = 1
			\end{equation}
			which becomes:
			\begin{equation}
				\bigg(\frac{\omega_n x_0}{\omega_n A}\bigg)^2  + \bigg(\frac{v_0}{\omega_nA}\bigg)^2 = 1
			\end{equation}
			Multiply each side by $(\omega_n A)^2$ to obtain:
			\begin{equation}
			...
			\end{equation}
			These relationships can be expressed as a trigonometric relationship
			\begin{figure}[H]
				\centering
				\includegraphics[]{../Figures/Trigonometric_relationship.png}
			\end{figure}
...
\begin{example}
			...
			A vehicle wheel, tire, and suspension can be modeled as a SDOF spring and mass. The mass is measured to be 300 kg, its frequency of oscillation is observed to be 10 rad/sec. What is the stiffness of the wheel assembly?\\
			
			\noindent\textbf{Solution:} Knowing $k=m\omega_n^2=(300$ $\text{kg})(10$ $\text{rad/s})^2=30$ KN/m   

\end{example}

\begin{example}		
			Consider the following 1-DOF system, where $k = 857.8$ N/m and $m=49.2\times10^{-3}$, calculate the natural frequency in rad/s and Hz. Also find the period of oscillations and the maximum displacement if the spring is initially displaced $10$ mm with no initial velocity.  	
			\begin{figure}[H]
				\centering
			...
			\end{equation}					
			To determine the maximum displacement, we can see that this will happen when sin($\omega_nt+\phi$)$=0$, therefore, the value of A is the maximum displacement. For an undamped system, $	A = \frac{\sqrt{\omega_n^2x_0^2+v_0^2}}{\omega_n}$,  
...
\subsection{General solution for vibrating systems}
		...
		\subsubsection{Review: complex numbers}
		...
		\subsubsection{Review:  Euler's formula.}
		...
		\subsection*{Formulate the general solution from Euler's expression. }						
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.8\linewidth]{../Figures/Trigonometric_relationship_A_theta_A1_A.png}
			\end{figure}
			again, $A$ and $\phi$ are:
			\begin{equation}
				A = \frac{\sqrt{\omega_n^2x_0^2+v_0^2}}{\omega_n} = \sqrt{x_0^2+\Bigg(\frac{v_0}{\omega_n}\Bigg)^2}
...
\end{example}
	...
	\subsection{Damping}
		...
		\subsection*{What is damping?}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[]{../Figures/dashpot.png}
			\end{figure}
		...
		\subsection*{What does it mean in real life?}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[]{../Figures/engine_mount.png}
			\end{figure}
...
\begin{example}				
			...
			\begin{figure}[H]
				\centering
				\includegraphics[]{../Figures/1-damped_DOF-mass_vertical.png}
			\end{figure}		
...
\begin{example}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[]{../Figures/rubber_mounted_mass.png}
			\end{figure}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[]{../Figures/rubber_mounted_mass_FBD.png}
			\end{figure}
%			\begin{equation}

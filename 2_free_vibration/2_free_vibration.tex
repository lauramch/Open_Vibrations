...
\begin{document}
	...
	\subsection{Mathematical Modeling of Free Vibration}
		...
		\subsubsection{Special considerations for $v_0=0$}
...
\begin{example}
			...
			\begin{equation}
				\omega_n = \sqrt{\frac{k}{m}}
			\end{equation}
			therefore, $k=m\omega_n^2=(300$ $\text{kg})(10$ $\text{rad/s})^2=30$ KN/m   
	...
	\end{review}
		...
		\subsubsection{Formulating the General solution for a 1-DOF Spring-Mass System}
			...
			\begin{equation}
				x(t) = ae^{\lambda t}
			\end{equation}
			is assumed where, $a$ and $t$ are nonzeros constants that need to be determined.  Using successive differentiation,the assumed sollution becomes:
			\begin{equation}
			...
			\end{equation}
			where $a_1$ and $a_2$ are complex valued constants of integration. This equation derived using Euler's formula is equivalent to the $A\text{sin}(\omega_n+\phi)$. To recover the previously assumed solution, the knowledge that $a_1$ and $a_2$ are complex congregate pairs and as such the magnitude can be expressed as $a_1=a_2$ is leveraged. Using Euler's polar notation, $a_1$ and $a_2$ can be expressed as 
			\begin{equation}
				a_1 = a_2 = ae^{j\psi}
			\end{equation}	
			...
			\end{equation}						
			This is equivalent to $	{x}(t) = A\text{sin}(\omega_n t + \phi)$ considering that $A=2a$ and knowing sin($\phi$) = cos($\phi + \psi$). Euler's equation states:
			\begin{equation}
				x(t) = a_1e^{+\omega_n j t} + a_2e^{-\omega_n j t}
			\end{equation}			
...
\end{example}
	...
	\subsection{Damping}
		...
		\subsubsection{Modeling Vibrating Systems with Damping}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[]{../figures/1-DOF-mass_horizontal_damping_FBD.png}
		...
		\subsubsection{Standard Form of the EOM}
			The EOM for a damped 1-DOF system is written in the form:
			\begin{equation}
				m\ddot{x}(t) + c \dot{x}(t) + kx(t) = 0
			\end{equation}
...
\begin{example}
			...
			\begin{figure}[H]
				...
				\caption{Representing a industrial damper as a spring-dashpot-mass system. }
			\end{figure}
